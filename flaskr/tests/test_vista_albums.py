from flaskr.vistas.vistas import VistaAlbumsUsuario_implementacion
import unittest
from flaskr.modelos.modelos import Medio
from flaskr.modelos import Album, db, Cancion, Usuario
from flaskr.vistas import VistaAlbumsUsuario
...
class test_vista_compartir_album(unittest.TestCase):
    ...
    def __init__(self, methodName):
        ...
        Faker.seed(2000)
        self.vista_albumes_usuarios = VistaAlbumsUsuario_implementacion()
        super().__init__(methodName=methodName)

    def test_obtener_lista(self):
        albumes_usuarios = self.vista_albumes_usuarios.get(self.usuario1.id)
        self.assertTrue(isinstance(albumes_usuarios, list))

    def test_lista_vacia(self):
        albumes_usuarios = self.vista_albumes_usuarios.get(self.usuario2.id)
        self.assertTrue(isinstance(albumes_usuarios, list))
